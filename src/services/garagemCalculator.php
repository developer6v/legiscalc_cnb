...
function calculatorGaragem($valor) {
    ...
    // MANTENDO SUA LÓGICA FINAL: garagem = 2/3 do valor do imóvel
    $factor = 0.666666667;

    return [
        'emolumento' => ($emolumento === 'undefined') ? 'undefined' : ($emolumento * $factor),
        'frj'        => ($frj === 'undefined') ? 'undefined' : ($frj * $factor),
        'issqn'      => ($issqn === 'undefined') ? 'undefined' : ($issqn * $factor),
        'total'      => ($total === 'undefined') ? 'undefined' : ($total * $factor),
    ];
}
